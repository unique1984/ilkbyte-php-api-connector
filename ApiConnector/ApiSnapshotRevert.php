...
final class ApiSnapshotRevert extends ApiConnector
{
    public function __construct(string $serverName, array $parameters = array(), $devMode = false)
    {
        $call = new Curl();
//        $url = sprintf(self::URL_API . self::URL_SNAPSHOT_REVERT, $serverName);
        $url = sprintf(self::URL_SNAPSHOT_REVERT, $serverName);
